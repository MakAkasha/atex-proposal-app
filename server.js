...
// Request logging
const logRequest = (method, pathname, status, responseTime) => {
    const logLevel = status >= 500 ? 'error' : status >= 400 ? 'warn' : 'info';
    const logMessage = `${new Date().toISOString()} [${logLevel.toUpperCase()}] ${method} ${pathname} ${status} ${responseTime}ms`;
    
    if (NODE_ENV === 'production') {
        // In production, write to file or use logging service
...
function securityHeaders(req, res, next) {
    ...
    res.setHeader('Content-Security-Policy', "default-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:;");
    ...
    res.writeHead = (statusCode, headers) => {
        if (statusCode === 200 && !compressed) {
            if (acceptEncoding.includes('gzip')) {
                ...
                res.end = ((end) => {
                    return function(chunk, encoding) {
                        ...
                        gzip.on('end', () => {
                            ...
                            originalEnd(compressedData, encoding);
                        });
                    };
                })(res.end);
                compressed = true;
                ...
                res.removeHeader('Content-Length');
                // Similar deflate logic could be added here
            }
        }
